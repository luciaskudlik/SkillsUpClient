  ...
  getSingleWorkshop = () => {
    ...
    axios.get(`http://localhost:5000/api/workshops/${id}`)
      .then((apiResponse) => {
        ...
        const { title, img, description, category, date, length, credits, participants, host, location } = theWorkshop;
        this.setState({ title, img, description, category, date, length, credits, participants, host, location });
      })
